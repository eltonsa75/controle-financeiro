...
export class CategoriaService {
  ...
  // Ajuste para a URL da sua API .NET
  private readonly API = 'http://localhost:5037/api/categorias';  

  constructor(private http: HttpClient) { }

  // Busca todas as categorias com o cálculo de gastos (BI)
  listarComStatus(mes: number, ano: number): Observable<CategoriaStatus[]> {
    return this.http.get<CategoriaStatus[]>(`${this.API}/status?mes=${mes}&ano=${ano}`);
  }
